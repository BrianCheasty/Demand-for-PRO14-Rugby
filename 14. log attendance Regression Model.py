...
savedfile=regResult1.to_csv(savepath+'(log)Attendance Regression.csv',index=False)
...
mapeDF=mapeDF.rename(columns={0:'Regression - (log)Attendance'})
savedfile=mapeDF.to_csv(savepath+'(log)Attendance RegressionMAPE.csv')
...
savedfile=regResult.to_csv(savepath+'Regression (log)Attendance Feature Selection Results.csv',index=False)

plt.plot(regResult['Mape Train'],color='steelblue',label='Train MAPE')
plt.xlabel('Features')
plt.ylabel('MAPE')
#plt.title('(log)Attendance - Reg - MAPE')
plt.legend(loc='upper right')
plt.hlines(y=regResult['Mape Train'].min(), xmin=0, xmax=cols, color='red', lw=.5)
plt.xlim([0, cols])
plt.savefig(savepath+'(log)Attendance Features vs Mape.jpeg')
plt.show()
...
featureRanks=featureRanks[(featureRanks['Ranking']< 42)]
savedfile=featureRanks.to_csv(savepath+'(log)Regression Feature Selection Rankings.csv',index=False)
...
# df=df.rename(columns={0:'Description',1:'(log)Attendance Regression'})
# savedfile=df.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/vif3.csv',index=False)

df=pd.DataFrame({'Without VIF':mapeTrain}.items())
df=df.rename(columns={0:'Description',1:'(log)Attendance Regression'})
savedfile=df.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/vif4.csv',index=False)
...
#plt.title('(log) Attendance - Reg - Residuals')
plt.legend()
plt.show()
...
pvalue=pd.DataFrame(['(log)Attendance Regression',p,x]).T
pvalue=pvalue.rename(columns={0:'Model',1:'p-value',2:'Interpretation'})
savedfile=pvalue.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/pvalue2.csv',index=False)
...
bp['Model']='(log)Attendance Regression'
...
savedfile=bp.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/bp2.csv',index=False)
...
mrresults2=['(log)Attendance Regression',rmse_train,rmse_test,r2_train,r2_test,mapeTrain,mapeTest]
...
savedfile=regResult3.to_csv(savepath+'(log)Attendance Regression Test.csv',index=False)
