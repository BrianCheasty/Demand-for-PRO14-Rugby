...
"""
dfAt=pd.read_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Feature Creation/Stadium Percentage Data.csv',\
                 encoding='latin')
...
#plt.title('Stadium Percentage Distribution Split')
plt.legend()
plt.show()

"""##########################
Model 3 Multiple Regression  on Stadium Percentage
"""
...
savedfile=regResult1.to_csv(savepath+'Stadium Percentage Regression.csv',index=False)
...
trainidate['APE']=np.sqrt((((trainidate['Stadium_Percentage']-trainidate['Predicted'])/trainidate['Stadium_Percentage'])*100)**2)
...
teamMAPE=[]
for i in team:
    df=trainidate[[i,'Stadium_Percentage','Predicted']]
    df=df[(df[i]==1)]
    mape=mean_absolute_percentage_error(df['Stadium_Percentage'],df['Predicted'])
    x=[{i:mape}]
    x=pd.DataFrame(x).T
    teamMAPE.append(x)
mapeDF=pd.concat(teamMAPE)
mapeDF=mapeDF.rename(columns={0:'Regression - Stadium Percentage'})
savedfile=mapeDF.to_csv(savepath+'Stadium Percentage RegressionMAPE.csv')
...
savedfile=regResult.to_csv(savepath+'Regression Stadium Percentage Feature Selection Results.csv',index=False)

plt.plot(regResult['Mape Train'],color='steelblue',label='Train MAPE')
plt.xlabel('Features')
plt.ylabel('MAPE')
#plt.title('Stadium Percentage - Reg - MAPE')
plt.legend(loc='upper right')
plt.hlines(y=regResult['Mape Train'].min(), xmin=0, xmax=cols, color='red', lw=.5)
plt.xlim([0, cols])
plt.savefig(savepath+'Stadium Percentage Features vs Mape.jpeg')
plt.show()
...
featureRanks=featureRanks[(featureRanks['Ranking']<28)]
savedfile=featureRanks.to_csv(savepath+'Regression Stadium Percentage Feature Selection Rankings.csv',index=False)
...
df=df.rename(columns={0:'Description',1:'Stadium Percentage Regression'})
savedfile=df.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/vif5.csv',index=False)

df=pd.DataFrame({'Without VIF':mapeTrain}.items())
df=df.rename(columns={0:'Description',1:'Stadium Percentage Regression'})
savedfile=df.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/vif6.csv',index=False)
...
trainResults['Error']=trainResults['Prediction']-trainResults['Stadium_Percentage']
...
#plt.title('Stadium Percentage - Reg - Residuals vs Fitted')
...
#plt.title('Stadium Percentage - Reg - Residuals')
plt.legend()
plt.show()
...
plt.legend(loc='upper left')
#plt.title('Stadium Percentage - Reg - Normal Q-Q')
...
pvalue=pd.DataFrame(['Stadium Percentage Regression',p,x]).T
pvalue=pvalue.rename(columns={0:'Model',1:'p-value',2:'Interpretation'})
savedfile=pvalue.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/pvalue3.csv',index=False)
...
bp['Model']='Stadium Percentage Regression'
...
savedfile=bp.to_csv('C:/Users/bcheasty/OneDrive - Athlone Institute Of Technology/Research Project/Data Set Creation/Data/Model/bp3.csv',index=False)
...
#plot_lm_3.axes[0].set_title('Stadium Percentage - Reg - Scale-Location')
...
df3=df.join(df2)
mrresults2=['Stadium Percentage Regression',rmse_train,rmse_test,r2_train,r2_test,mapeTrain,mapeTest]
...
savedfile=regResult3.to_csv(savepath+'Stadium Percentage Regression Test.csv',index=False)
